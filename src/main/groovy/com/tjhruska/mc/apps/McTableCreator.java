...
public class McTableCreator extends BeanNameAwareRunnable {
	private static final Logger log
		= LoggerFactory.getLogger(McTableCreator.class);
	...
	public static McTableCreator configure(){
		ApplicationContext context = Context.getClassPathXmlApplicationContext("conf/mcTableCreator/application_context.xml");
		return (McTableCreator)context.getBean("mcTableCreator");
	}
	...
	public static class CommandBuilder{
			...
			List<CommandFragment> commandFragments, Map<String, String> existingTablesAndViews) 
		{
			...
			Long prevItem = 0L;
			do {
				CommandFragment commandFragment = i.next();
				log.debug("{}-{}-{}", new Object[]{commandFragment.mcTableId, commandFragment.section, commandFragment.sequence});
				...
				if (i.hasNext() && (!prevItem.equals(commandFragment.mcTableId))){
				...
				if (prevCommand != null &&
					(!i.hasNext()
					|| !prevItem.equals(commandFragment.mcTableId)))
				{
					log.debug("More rows={}, if false or record changed, then save off {}", new Object[]{new Boolean(i.hasNext()).toString(), prevItem.toString()});
		...
		public static void writeOutPendingCommands(StringBuilder directoryName, 
