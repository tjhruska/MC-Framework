...
public class McEnumDbSynchronizer extends BeanNameAwareRunnable {
	...
	private boolean skipEnumToColumnTableNextRun = false;
	public void setSkipEnumToColumnTableNextRun(){
		skipEnumToColumnTableNextRun = true;
	}
	...
	public static McEnumDbSynchronizer configure(){
		ApplicationContext context = Context.getClassPathXmlApplicationContext("conf/mcEnumDbSynchronizer/application_context.xml");
		(McEnumDbSynchronizer)context.getBean("mcEnumDbSynchronizer");
	}
	...
	public void run() {
		...
		if (skipEnumToColumnTableNextRun){
			log.info("Skipping the load of enumerations to mc_column table this run.");
			skipEnumToColumnTableNextRun = false;
		}else{
			log.info("Synchronizing the enumeration table to mc_table and mc_column");
	...
	public static class CommandBuilder{
      ...
      commandFragments.each { commandFragment -> 
					...
					|| !prevItem.getTwo().equals(commandFragment.id)))
				{
					...
					sqlTemplate.setSimpleJdbcTemplate(buildEnumSynchCommands.getSimpleJdbcTemplate());
