...
class McFileGenerator extends BeanNameAwareRunnable {
	...
	private List<SQLTemplate> fragmentQueries;
	public void setFragmentQueries(List<SQLTemplate> fragmentQueries){
		this.fragmentQueries = fragmentQueries;
	}
	
	private Map<String, String> installProperties;
	public void setInstallProperties(Map<String, String> installProperties){
	...
	public static McFileGenerator configure(){
		ApplicationContext context = Context.getClassPathXmlApplicationContext("conf/mcFileGenerator/application_context.xml");
		return (McFileGenerator)context.getBean("mcFileGenerator");
	}
    ...
    //if not groovy, pull the pogo command if it is present
      ...
      fragmentQueries.removeAll {
        it.getBeanName().equals("getPogoCodeFragments")
      }
    ...
    //if not java, pull the pojo command if it is present
      ...
      fragmentQueries.removeAll {
        it.getBeanName().equals("getPojoCodeFragments")
      }
    ...
    fragmentQueries.each { fragmentQuery ->
			String beanName = fragmentQuery.getBeanName();
			log.debug("Found fragment query {}.", fragmentQuery.getBeanName());
    ...
    fileFragments.each { k, v ->
        ...
        [k, Integer.toString(v.size()), Integer.toString(assembledFiles.get(k).size())].toArray());
		}
    ...
    assembledFiles.each { k, v ->
			...
			log.info("For {}, backedUp {} files to {}.", [k, backupCount, installProperties.get("backupDirectory")].toArray());
			int filesWritten = writeOutFiles(v);
			log.info("For {} wrote out {} files to {}.", [k, Integer.toString(filesWritten), installProperties.get("rootDirectory")].toArray());
	...
	private List<FileFragment> getFileFragments(SQLTemplate sqlTemplate){
		...
		log.trace("about to run {} query:\n{}", sqlTemplate.getBeanName(), sqlTemplate.applyTags(installProperties));
		try{
			...
			log.error("Failed to retrieve command fragements using {}", sqlTemplate.getBeanName());
    ...
    fileFragments.each { fileFragment ->
			log.trace("ID-{}, sequence-{}", [fileFragment.id+"", fileFragment.sequence+""].toArray());
			...
			{
				log.trace("More rows={}, if false or record changed, then save off ID-{}", [(!fileFragment.equals(fileFragments.last())).toString(), prevFile.id.toString()].toArray());
				...
				if (fileFragment.equals(fileFragments.last()) && prevFile.id != currentFile.id){
					...
					log.trace("More rows={}, if false or record changed, then save off ID-{}", [(!fileFragment.equals(fileFragments.last())).toString(), currentFile.id.toString()].toArray());
				}
			}
    ...
    assembledFiles.each { assembledFile ->
		    ...
		    if (file.exists()) {
		    	...
		    	backupCount++;
		    	log.debug("backed up file {} to: {}", fqfName, fqfNameBackup);
		    	
		    }else{
		    	log.debug("no backup needed, no prev ver of file exists: {}", fqfName);
		    }
    ...
    assembledFiles.each { assembledFile ->
			...
			try{
				...
				writeFileCount++;
				log.debug("wrote file: {}{}", directoryName, fileName);
